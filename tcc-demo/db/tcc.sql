...
CREATE TABLE `tcc_tx` (
  `xid` varchar(40) NOT NULL COMMENT '全局事务id',
...
CREATE TABLE `tcc_tx_child` (
  ...
  `dubbo_version` VARCHAR(24) DEFAULT null COMMENT 'dubbo version',
  `commit_method` VARCHAR(120) DEFAULT null COMMENT 'commit 方法名',
  `rollback_method` VARCHAR(120) DEFAULT null COMMENT 'rollback 方法名',
  `parames_types` VARBINARY(256) DEFAULT null COMMENT '请求参数类型列表',
  `parames_values` VARBINARY(2048) DEFAULT null COMMENT '请求参数值 序列化',
